...
// draw calendar function
	...
	//	echo $current_month_start_as_day . "<br />";
	...
	// find 1st of next month as day of year by making yy-mm-dd format and doing strtotime(). If month is December, find last day of December instead.
		...
		//	echo $next_month_start_as_day . "<br />";
...
<html>
<head>
   <title>ADMIN Bookings Calendar</title>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$('#maincalendar .available-day').hover(function() {$(this).find("p.availabletext").html("Disable!");} , function() {$(this).find("p.availabletext").html("Available!");});
			...
			$('#maincalendar .booked-day').click(function() {
				...
				$.post ('bookinglookup.php',
				 	{ current_year: current_year, datestring: datestring },
					function(data) {
			...
			$('#disable').click(function() {
					...
					$.post ('newbooking.php',
							{ arrivaldate: arrivaldate, length_of_stay: length_of_stay, firstname: firstname, lastname: lastname},
							function(data) {
								$('#container').html(data);
...
<body>
	<div id="allcontent">
		...
		<p style="color: red;"><strong>YOU ARE IN ADMIN MODE</strong> - <a style="text-decoration: underline" href="calendar.php">Back to normal calendar</a></p>
<?php 

// RIDICULOUSLY REDUNDANT REPEATING OF CODE BECAUSE I CBF - Makes sure the month is a real month and disables "previous" buttons when it's the current month.
...
// if ($month <= date("n")) {
// 	$month = date("n");
// 	echo '<center><h2><span style="display:none;" id="current_month">' . $month . '</span> <span id="monthname" style="margin-left: 10px;">' . date('F',mktime(0,0,0,$month,1,$year)).'</span>' . ' <span id="current_year" style="margin-right: 10px;">' . $year . '</span> <a class="control" href="calendar.php?month='. ( $month != 12 ? $month + 1 : 1) .'&year='. ( $month != 12 ? $year : $year + 1 ) .'">Next month &raquo;</a> <a class="control" href="calendar.php?month='. date("n").'&year='. date("Y") .'">Today</a></h2></center>';
// 
// } elseif ($month > 12) {
// 	$month = 12;
// 	echo '<center><h2><a class="control" href="calendar.php?month='. ( $month != 1 ? $month - 1 : 12) .'&year='. ( $month != 1 ? $year : $year - 1 ) .'">&laquo; Prev month</a> <span style="display:none;" id="current_month">' . $month . '</span> <span id="monthname" style="margin-left: 10px;">' . date('F',mktime(0,0,0,$month,1,$year)).'</span>' . ' <span id="current_year" style="margin-right: 10px;">' . $year . '</span> <a class="control" href="calendar.php?month='. ( $month != 12 ? $month + 1 : 1) .'&year='. ( $month != 12 ? $year : $year + 1 ) .'">Next month &raquo;</a> <a class="control" href="calendar.php?month='. date("n").'&year='. date("Y") .'">Today</a></h2></center>';
// 
// } else {
	echo '<center><h2><a class="control" href="admincalendar.php?month='. ( $month != 1 ? $month - 1 : 12) .'&year='. ( $month != 1 ? $year : $year - 1 ) .'">&laquo; Prev month</a> <span style="display:none;" id="current_month">' . $month . '</span> <span id="monthname" style="margin-left: 10px;">' . date('F',mktime(0,0,0,$month,1,$year)).'</span>' . ' <span id="current_year" style="margin-right: 10px;">' . $year . '</span> <a class="control" href="admincalendar.php?month='. ( $month != 12 ? $month + 1 : 1) .'&year='. ( $month != 12 ? $year : $year + 1 ) .'">Next month &raquo;</a> <a class="control" href="admincalendar.php?month='. date("n").'&year='. date("Y") .'">Today</a></h2></center>';	
// }
...
<div id="clickcalendartext">
	<center><p><em>Click an <span style="color: green;"><strong>available date </strong></span>to <strong>disable reservations</strong> for a certain period.</p>
		<p>Click on a <span style="color: red;"><strong>booked date </strong></span>to <strong>display information</strong> about that booking.</em></p></center>
</div>
<div id="makebooking">
	<center><h2>Disable reservations</h2></center>
	<div id="firstinfo">
		<center>
		<form id="firstinfoform">
		<table>
			<tr>
				<th>From:</th><td><input type="text" class="date" id="arrivaldate" name="newarrivaldate" value="Click to select"></td>
			</tr><tr>
				<th>For how many nights?</th><td><select id="length_of_stay" name="newlength">				
			...
			<tr>
				<th></th><td><input type="button" id="disable" value="Disable"></td>
			</tr>
		</table>
		</form>
		</center>	
